...
async function postChangePhotoProfile({ photoFile }) {
  ...
  _fetchWithAuth(`${BASE_URL}/users/photo`, {
  method: 'POST',
  body: formData,
  });
  const responseJson = await response.json();
  ...
  const { data: { todo_id } } = responseJson;
  return todo_id;
  }
  ...
  const response = await
  _fetchWithAuth(`${BASE_URL}/todos/${id}/cover`, {
  method: 'POST',
  body: formData,
  });
  const responseJson = await response.json();
  ...
  async function putUpdateTodo({ id, title, description,
  is_finished }) {
  const response = await
  _fetchWithAuth(`${BASE_URL}/todos/${id}`, {
  method: 'PUT',
  headers: {
  'Content-Type': 'application/json',
  },
  ...
  const response = await
  _fetchWithAuth(`${BASE_URL}/todos?is_finished=${is_finished}`);
  const responseJson = await response.json();
  const { success, message } = responseJson;
if (success !== true) {
throw new Error(message);
}
const { data: { todos } } = responseJson;
return todos;
}
